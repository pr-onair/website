...
/* @gov.au/animate v1.0.11 */
var AU=AU||{};(function(AU){var animate={};function CalculateAnimationSpecs(initialSize,endSize,speed){if(initialSize===endSize){return{stepSize:0,steps:0,intervalTime:0}}var distance=endSize-initialSize;var intervalTime=speed/distance;var stepSize=distance<0?-1:1;var steps=Math.abs(distance/stepSize);intervalTime=speed/steps;if(Math.abs(intervalTime)<1e3/60){intervalTime=1e3/60;steps=Math.ceil(Math.abs(speed/intervalTime));stepSize=distance/steps}return{stepSize:stepSize,steps:steps-1,intervalTime:intervalTime}}if(typeof module!=="undefined"){animate.CalculateAnimationSpecs=CalculateAnimationSpecs}animate.GetCSSPropertyBecauseIE=function(element,property){if(typeof getComputedStyle!=="undefined"){return window.getComputedStyle(element)[property]}else{var space=element.currentStyle[property];if(space==="auto"){space=AU.animate.CalculateAuto(element,property)}return space}};animate.CalculateAuto=function(element,dimension){var initialSize;var endSize;if(dimension==="height"){initialSize=element.clientHeight;element.style[dimension]="auto";endSize=element.clientHeight;element.style[dimension]=initialSize+"px"}else{initialSize=element.clientWidth;element.style[dimension]="auto";endSize=element.clientWidth;element.style[dimension]=initialSize+"px"}return parseInt(endSize)};animate.Stop=function(element){clearInterval(element.AUanimation)};animate.Run=function(options){var elements=options.element;var speed=options.speed||250;if(elements.length===undefined){elements=[elements]}if(typeof options.callback!=="function"){options.callback=function(){}}elements[0].AUinteration=0;elements[0].AUinterations=elements.length;for(var i=0;i<elements.length;i++){var element=elements[i];AU.animate.Stop(element);var initialSize=parseInt(AU.animate.GetCSSPropertyBecauseIE(element,options.property));var endSize=options.endSize;if(options.endSize==="auto"){endSize=AU.animate.CalculateAuto(element,options.property)}var animationSpecs=CalculateAnimationSpecs(initialSize,endSize,speed);var iterateCounter=initialSize;if(animationSpecs.stepSize<0){element.AUtoggleState="closing"}else if(animationSpecs.stepSize>0){element.AUtoggleState="opening"}(function(element,initialSize,iterateCounter,animationSpecs,endSize){element.AUanimation=setInterval(function(){if(initialSize===endSize||animationSpecs.steps===0){AU.animate.Stop(element);element.style[options.property]=endSize+"px";element.AUtoggleState="";elements[0].AUinteration++;if(elements[0].AUinteration>=elements[0].AUinterations){return options.callback()}}else{iterateCounter+=animationSpecs.stepSize;element.style[options.property]=iterateCounter+"px";animationSpecs.steps--}},Math.abs(animationSpecs.intervalTime))})(element,initialSize,iterateCounter,animationSpecs,endSize)}};animate.Toggle=function(options){var elements=options.element;var property=options.property||"height";var speed=options.speed||250;var closeSize=options.closeSize||0;var openSize=options.openSize||"auto";if(elements.length===undefined){elements=[elements]}if(typeof options.prefunction!=="function"){options.prefunction=function(){}}if(typeof options.postfunction!=="function"){options.postfunction=function(){}}if(typeof options.callback!=="function"){options.callback=function(){}}elements[0].AUtoggleInteration=0;elements[0].AUtoggleInterations=elements.length;for(var i=0;i<elements.length;i++){var element=elements[i];AU.animate.Stop(element);var targetSize;var preState="";var postState="";var currentSize=parseInt(AU.animate.GetCSSPropertyBecauseIE(element,options.property));if(currentSize===closeSize||element.AUtoggleState==="closing"){targetSize=openSize;preState="opening";postState="open"}else if(currentSize!==closeSize||element.AUtoggleState==="opening"){targetSize=closeSize;preState="closing";postState="closed"}else{throw new Error("AU.animate.Toggle cannot determine state of element")}options.prefunction(element,preState);AU.animate.Run({element:element,endSize:targetSize,property:property,speed:speed,callback:function(){elements[0].AUtoggleInteration++;if(elements[0].AUtoggleInteration===elements[0].AUinterations){var returnParam=options.callback(element,postState);options.postfunction(element,postState);return returnParam}options.postfunction(element,postState)}})}};AU.animate=animate})(AU);if(typeof module!=="undefined"){module.exports=AU}if(typeof exports!=="undefined"){Object.defineProperty(exports,"__esModule",{value:true});eval("exports.default = AU")}
...
gumshoe.init({
	selector: '.js-scrollspy',
	selectorHeader: '.js-scrollspy-wrapper',
	activeClass: 'is-scrolledto',
});
...
document
	.querySelectorAll('.js-expand')
	.forEach( function( element ) {
		element.addEventListener('click', function( event ) {
			event.preventDefault();
			var i = element.getAttribute('data-expand');
			var s = element.getAttribute('data-section');
			var t = document.querySelector( i );

			AU.animate.Run({
				element: document.querySelectorAll('.js-expandable-' + s + ':not(' + i + ')'),
				property: 'height',
				endSize: 0,
				speed: 300,
			});
